...
def main() -> int:
    s3 = s3_client(); b = bucket(); head = s3.head_object(Bucket=b, Key=KEY)
    ...
    if isinstance(ckpt, dict):
        for k in ("model_state_dict", "state_dict", "model"):
            if isinstance(ckpt.get(k), dict): state_key = k; state = ckpt[k]; break
    out = {
        "key": KEY,
        ...
        "position_embed_shape": list(state["position_embed.weight"].shape) if isinstance(state, dict) and "position_embed.weight" in state else None,
        "range_requests": reader.range_requests,
        "bytes_fetched": reader.bytes_fetched,
    }
    Path("v14a2_original_checkpoint_probe.json").write_text(json.dumps(out, indent=2, sort_keys=True), encoding="utf-8")
